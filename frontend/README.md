...
| Flujo OIDC "Authorization Code con PKCE" | [`src/auth/AuthContext.jsx:114-122`](src/auth/AuthContext.jsx#L114-L122) y [`src/auth/oidcConfig.js:5-16`](src/auth/oidcConfig.js#L5-L16) | `loginPostor()`/`loginStaff()` llaman a `signinRedirect()` de `oidc-client-ts`, configurado con `response_type: "code"` — la librería genera el `code_verifier`/`code_challenge` (PKCE) y valida `state` internamente, no hay que armarlo a mano. Confirmado en producción con los dos proveedores |
| Completa el flujo tras volver del IdP, obtiene los tokens y provisiona el perfil | [`src/pages/CallbackPostorPage.jsx:11-22`](src/pages/CallbackPostorPage.jsx#L11-L22), [`src/pages/CallbackStaffPage.jsx:11-20`](src/pages/CallbackStaffPage.jsx#L11-L20) y [`src/auth/AuthContext.jsx:124-138`](src/auth/AuthContext.jsx#L124-L138) | `completarCallbackPostor()`/`completarCallbackStaff()` llaman a `signinRedirectCallback()`, que intercambia el `code` por los tokens (id/access/refresh); apenas eso resuelve, cada página llama una vez a `GET /usuarios/me` para dejar provisionado el perfil desde el primer login, sin esperar a que el usuario visite "Mi perfil" |
| Extrae claims del token (rol, nombre, email, sub) | [`src/auth/AuthContext.jsx:51-66`](src/auth/AuthContext.jsx#L51-L66) y [`src/auth/oidcConfig.js:48-50`](src/auth/oidcConfig.js#L48-L50) | `sessionFromOidcUser()` arma la sesión desde `user.profile` y decide qué token mandar al backend (ver "Autenticación" más arriba); `extraerRol()` centraliza de dónde sale el rol (`roles[0]` para Entra ID; Cognito no tiene claim de rol, se asume `POSTOR` por proveedor) |
